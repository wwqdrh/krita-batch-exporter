...
def exportSelectedLayersWithSameSize(cfg, statusBar):
    msg, timeout = (
        cfg["done"]["msg"].format("Exported selected layers with same size."),
        cfg["done"]["timeout"],
    )
    try:
        doc = KI.activeDocument()
        ...
        # Convert nodes to WNodes and find the maximum size
        wnodes = list(map(partial(WNode, cfg), nodes))
        max_width = max(node.size[0] for node in wnodes)
        max_height = max(node.size[1] for node in wnodes)

        # Export each layer with the maximum size
        for node in wnodes:
            node.save_with_size(dirName, target_size=(max_width, max_height))
...
class GameArtTools(DockWidget):
    ...
    def createInterface(self):
        ...
        exportSelectedLayersSameSizeButton = QPushButton("Selected Layers (Same Size)")
        ...
        exportSelectedLayersSameSizeButton.setToolTip("Export selected layers with same size")
        ...
        vboxlayout.addWidget(exportSelectedLayersSameSizeButton)
        ...
        exportSelectedLayersSameSizeButton.released.connect(
            partial(exportSelectedLayersWithSameSize, CONFIG, statusBar)
        )
