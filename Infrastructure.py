...
class WNode:
    ...
    def save_with_size(self, dirname="", target_size=None):
        """
        Transform Node to a QImage with a specific target size
        processes the image, names it based on metadata, and saves the image to the disk.
        ...
        if target_size:
            new_img = QImage(target_size[0], target_size[1], QImage.Format_ARGB32)
    ...
    def saveCOASpriteSheet(self, dirname=""):
        ...
        for count, image in enumerate(images):
            ...
            painter.drawImage(
                coord_rel_x,
                image_height * count + coord_rel_y,
                nodeToImage(image),
            )
